/* -----------------------------------------
   invoice.js – منطق الفواتير (مفرق + جملة)
------------------------------------------*/
...
// رصيد العميل من الفواتير المخزنة محلياً
...
function addItemToCurrentInvoice(barcode, qty, type){
  ...
  // خصم من المخزون محلياً
  product.stock = (product.stock || 0) - qty;

  renderInvoiceItemsTable(type);
  updateInvoiceTotals(type);
  renderStockTable($("#stock-search") ? $("#stock-search").value : "");
}
...
/* -------------------------
   حفظ الفاتورة (مع إرسال لـ WebApp)
--------------------------*/
async function saveCurrentInvoice(type, paymentOnly = false){
  ...
  const customerType = $(accountTypeSelect).value;   // cash / credit
  ...
  // إضافة الفاتورة لقائمة الفواتير المحلية (للاستخدام في كشف الحساب)
  ...
  if(res.status === "success"){
    alert(paymentOnly ? "تم تسجيل الدفعة بنجاح." : "تم حفظ الفاتورة في قاعدة البيانات.");
  } else {
    // فشل الاتصال أو خطأ في السيرفر – خزّن الفاتورة للأوفلاين
    queueInvoiceForSync(AppState.currentInvoice);
    alert("تم حفظ الفاتورة محلياً (أوفلاين). سيتم إرسالها تلقائياً عند توفر الإنترنت.");
  }

  // إنشاء فاتورة جديدة لنفس النوع
  createNewInvoice(type);
...
function runStatement(){
  ...
  filtered.forEach(inv => {
    ...
    tr.innerHTML = `
      <td>${inv.dateISO}</td>
      <td>${inv.id}</td>
      <td>${inv.type === "retail" ? "فاتورة زبون" : "فاتورة تاجر"}</td>
      <td class="text-right">${inv.items && inv.items.length ? "فاتورة بيع" : "سند قبض"}</td>
      <td>${fnum(debit)}</td>
      <td>${fnum(credit)}</td>
      <td>${fnum(balance)}</td>
    `;
...
function renderQuickButtons(){
  ...
  // الصفوف 2–4: أعلى 9 أصناف مبيعاً بدون تكرار
